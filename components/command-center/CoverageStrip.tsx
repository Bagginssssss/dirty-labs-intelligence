...
// filled = covered (green), gap = missing expected period (red), neutral = quiet week
// for event-driven / irregular snapshot reports (faint slate — not "missing data").
    ...
    <div className="flex gap-[2px] mt-[8px]">
      {cells.map(c => (
        <div
          ...
          style={{ backgroundColor: CELL_COLOR[c.state], opacity: c.state === 'neutral' ? 0.55 : 1 }}
        />
      ))}
    </div>
  )
